...
/**
 * Default constructor, doesn't recive any parameters and start with default
 * values
 */
Button::Button() {}
...
void Button::setBackgroundColor(sf::Color color) {
  this->insideRect.setFillColor(color);
}
...
void Button::setOutlineSize(float size) {
  ...
  this->outlineRect.setSize(
      sf::Vector2f(this->insideRect.getSize().x + 2 * size,
...
void Button::setOutlineColor(sf::Color color) {
  this->outlineRect.setFillColor(color);
}
 ...
 * @return void
 */
void Button::setCallbackFunction(function<void()> function) {
...
void Button::setColorOnHover(btnElement elem, sf::Color newColor) {
  switch (elem) {
    ...
    case BACKGROUND:
      this->hoverFunctions.push_back([this, newColor]() -> void {
        this->insideRect.setFillColor(newColor);
      });
      break;

    case OUTLINE:
      this->hoverFunctions.push_back([this, newColor]() -> void {
        this->outlineRect.setFillColor(newColor);
      });
      break;
  }
}
 ...
 * @public
 * @param originPos A Vector2F param that has max value 1 and min 0. Recives a %
 * of where may the origin point be located.
 * @return void
 */
